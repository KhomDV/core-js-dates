...
function getNextFriday(date) {
  ...
  date.setDate(date.getDate() + (day < 5 ? 5 - day : 7 - (day - 5)));
  return date;
}
...
function isDateInPeriod(date, period) {
  const start = Number(period.start.replaceAll('-', ''));
  const end = Number(period.end.replaceAll('-', ''));
  const datePeriod = Number(date.replaceAll('-', ''));
  return datePeriod >= start && datePeriod <= end;
}
...
function formatDate(date) {
  ...
  return `${newDate.getMonth() + 1}/${newDate.getUTCDate()}/${newDate.getFullYear()}, ${newDate.toLocaleTimeString('en-US', { timeZone: 'UTC' })}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= lastDay; i += 1) {
    const dayWeek = new Date(year, month - 1, i).getDay();
...
function getWeekNumberByDate(date) {
  const dateNew = new Date(date.getFullYear(), 0, 1);
  const dateDiff = (date - dateNew) / (24 * 60 * 60 * 1000);
  const result = Math.ceil((dateDiff + dateNew.getDay()) / 7);
  return dateNew.getDay() === 0 ? result + 1 : result;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = month; i < 12; i += 1) {
    const resultDate = new Date(year, i, 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const currentDate = startPeriod;
  const arrDate = [];

  while (currentDate <= endPeriod) {
    for (let i = 1; i <= countWorkDays; i += 1) {
      arrDate.push(currentDate.toLocaleDateString('es-CL'));
      currentDate.setDate(currentDate.getDate() + 1);
      if (currentDate > endPeriod) break;
    }
